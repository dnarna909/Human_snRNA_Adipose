...
if (dim(adata_temp)[2]>0){
  ...
  if (flag_sex==TRUE){
    ...
    # Summarize results 
    summaryDt <- summaryCond_BMI$datatable
    dt1 = summaryDt[contrast=="BMI" & component=="H", .(primerid, `Pr(>Chisq)`)]
    dt2 = summaryDt[contrast=="BMI" & component=="logFC", .(primerid, coef, z)]
    de_res = merge(dt1, dt2, by="primerid")
    colnames(de_res) <- c("gene", "BMI.H_p", "BMI.logFC", 'BMI.logFC_z')
    de_res$BMI.H_fdr <- p.adjust(de_res$BMI.H_p, "fdr")
  }
