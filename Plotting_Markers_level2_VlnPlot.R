...
for (markers.file in markers.files) {
  ...
  seurat.file = seurat.files[match(markers.file, markers.files)]
  
  # Load data.frame obj
  seurat.data <- readRDS(paste0(Disk, Project.folder, "/", Rds.folder, "/", seurat.file) )
  file.name <- stringr::str_split(seurat.file, ".Rds")[[1]][1];file.name
  Annotation.file.name <- paste(stringr::str_split(file.name, "_")[[1]][c(1,2,3)], collapse = "_");Annotation.file.name
  ...
  for (nn in names(Result[[ll]]) ) {
    ...
    if (nrow(Result[[ll]][[nn]]%>% dplyr::filter(Direction %in% c("Up", "Down")))  > 0 ) {
      n = 25
      features = unique(c(
        Result[[ll]][[nn]] %>% dplyr::filter(Direction == "Up") %>% dplyr::top_n(-n, Rank.ID) %>% pull(rowname),
        Result[[ll]][[nn]] %>% dplyr::filter(Direction == "Down") %>% dplyr::top_n(-n, Rank.ID.neg) %>% pull(rowname)));features
      ...
      for (mm  in names(pbmc.melt)) {
        ...
        a <- ggplot(pbmc.melt[[mm]], aes(factor(Idents), Expr, fill = Idents)) +
          ...
          ggtitle(paste0(nn, " Markers from ", ll, " Cells and plotted in ", mm, " Cells")) + 
          # xlab("Identity") + 
        ...
        png(filename = paste0(dir, 
                              ...
                              paste0(ll, ".", nn, ".",mm), "_l2_VlnPLot.png"), 
            width =10, 
...
rm(a, all.pbmc, df, metadata, pbmc, pbmc.melt,Annotation.file.name,
   ...
   markers.file, mm, n, nn,seurat.file)
gc()
sessionInfo()