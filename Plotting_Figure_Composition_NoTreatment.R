  ...
  for (list.name in names(Composition.lists)) {
    ...
    Composition_3 <- Composition.lists[[list.name]][["Composition_3All"]] %>% 
      dplyr::filter(Treatment_Group %in% c("Notreatment", "NUT_Pre", "SGLT2i_Pre"),
                    BA_Group %in% c("Diabetes", "Older_Lean", "Older_Overweight", "Older_PreD_Obese")
                    ) %>% 
    ...
    ggboxplot(Composition_3, x = "BA_Group", y = "composition",
              color = "BA_Group", palette = "npg",
              add = "jitter",
              facet.by = c( label.name), short.panel.labs = T
              )+
      stat_compare_means(aes_string(group=label.name), 
