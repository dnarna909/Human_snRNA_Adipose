...
for (file in files) {
  ...
  for (list.name in names(Composition.lists)) {
    ...
    Composition_3 <- Composition.lists[[list.name]][["Composition_3All"]] %>% 
      dplyr::filter(
        !!sym(Select.Group) %in% Select.Group.names #"Middle_Lean", "Middle_Overweight" 
      )  
    Composition_3[[Select.Group]] <- factor(Composition_3[[Select.Group]], levels = Select.Group.names) # "Middle_Lean", "Middle_Overweight"
