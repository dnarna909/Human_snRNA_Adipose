---
title: "Main_04_FAP_Fat_1.for annotation"
author: "Jia Nie"
date: "1/13/2022"
...
gc() #free up memrory and report the memory usage.
seurat.data <- subset(eWAT, subset = Annotation == "FAP")
rm(eWAT)
gc() #free up memrory and report the memory usage.
...
gc() #free up memrory and report the memory usage.
seurat.data <- FindVariableFeatures(seurat.data, nfeatures=3000) #sarvar 2000
...
seurat.data <- RunHarmony(seurat.data, group.by.vars="Dataset", assay.use = "originalexp")# ,max.iter.harmony = 20
gc() #free up memrory and report the memory usage.
ElbowPlot(seurat.data, ndims = 50) # choose dims
seurat.data <- RunUMAP(seurat.data, dims=1:20, reduction="harmony")
seurat.data <- FindNeighbors(object = seurat.data, dims = 1:20, reduction = "harmony") # from Main_05_Adipocytes_Fat_1_Step1.rmd
