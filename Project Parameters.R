...
# Load libraries 
packageList <- scan( paste0(Disk, "00_Functions_Refs/SingleCellPreprocessPackageList1.txt") , character(), quote = "")
for(package in packageList){
  if(!require(package,character.only = TRUE)){
    install.packages(package);require(package,character.only = TRUE);}
}
packageList <- scan( paste0(Disk, "00_Functions_Refs/SingleCellPreprocessPackageList2.txt") , character(), quote = "")
for(package in packageList){
  if(!require(package,character.only = TRUE)){
    BiocManager::install(package);require(package,character.only = TRUE);}
}
packageList <- scan( paste0(Disk, "00_Functions_Refs/SingleCellPreprocessPackageList3.txt") , character(), quote = "")
for(package in packageList){
  if(!require(str_split(package, "/")[[1]][2], character.only = TRUE)){
    devtools::install_github(package);require(str_split(package, "/")[[1]][2],   character.only = TRUE);}
}
