...
source(paste0(Disk, "00_Functions_Refs/Create Matrices Functions for Single Cell data.R"), local = knitr::knit_global())
...
# Set up folder ------------------------------------------------------------------------
dir.create(file.path(paste0(Disk, Project.folder, "/", figures.folder, "/",  "velocyto", "/")), showWarnings = FALSE)
dir0 <- paste0(Disk, Project.folder, "/", figures.folder, "/", "velocyto",  "/")
dir.create(file.path(paste0(Disk, Project.folder, "/", Rds.folder, "/",  "velocyto", "/")), showWarnings = FALSE)
dir1 <- paste0(Disk, Project.folder, "/", Rds.folder, "/",  "velocyto",  "/")

### Import data
Adipocytes <- readRDS("/media/jianie/Extreme SSD/2021-01-11 Figures for Grant resubmission/Rds files/Adipocytes.Rds")
FAP <- readRDS("/media/jianie/Extreme SSD/2021-01-11 Figures for Grant resubmission/Rds files/FAP.Rds")
Amb <- readRDS("/media/jianie/Extreme SSD/2021-01-11 Figures for Grant resubmission/Rds files/Ambient.Rds")

### RNA Velocity in FAP compartment
# Importing raw zUMIs output
##' Import count objects--------------------------------------------------------------------------------------------------------
matrices.open(filefolders = counts.subfolder, folder.route=paste0(Disk, counts.folder))
names(matrices.All)
...
file<- readRDS(file ="counts.matrices.rds")

bm[["spliced"]] <- CreateAssayObject( counts = file$exon )
bm[["unspliced"]] <- CreateAssayObject( counts = file$unspliced )
bm[["spanning"]] <- CreateAssayObject( counts = file$ambiguous )
