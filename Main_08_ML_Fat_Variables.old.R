  ...
  for (list.name in names(Composition.lists)) {
    ...
    Composition_3 <- Composition.lists[[list.name]][["Composition_3All"]] %>% 
      dplyr::filter(Treatment_Group %in% c("Notreatment", "NUT_Pre", "SGLT2i_Pre"),
                    BA_Group %in% c("Middle","Older_Lean", "Older_Overweight", "Older_PreD_Obese", "Diabetes")#
      ) %>% 
      mutate(BA_Group = factor(BA_Group, levels = c("Middle","Older_Lean", "Older_Overweight", "Older_PreD_Obese", "Diabetes")))# 
